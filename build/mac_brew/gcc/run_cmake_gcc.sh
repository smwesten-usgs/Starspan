...
export GCCLIST=$( glocate gcc-6 | grep Cellar | grep bin | grep $GCC_VERSION | grep -v 386 )
export GCCARR=($GCCLIST)
export CC=${GCCARR[1]}

export GXXLIST=$( glocate g++-6 | grep Cellar | grep bin | grep $GCC_VERSION | grep -v 386 )
...
export LIB_GDAL=$(glocate libgdal.dylib | grep Cellar )
...
export CMAKE_CXX_FLAGS_DEBUG="-O0 -g -ggdb $(gdal-config --cflags)"
export CMAKE_CXX_FLAGS_RELEASE="-O2 -mtune=native -Wno-maybe-uninitialized $(gdal-config --cflags)"
